...
def get_player_name():
    """
    Function to get a chosen name from the user and run length validation.
...
def validate_player_name(name):
    """
    Checks that the player enters at least one character as a name.
    Also checks that the length of the name is no more than 16 characters so it
    will fit on the congratulations window if they make it to the end.

    The name can be anything they want, so there is no validation to check if
    they have entered any sybmols or numbers.
    """
    try:
        length = len(name)
...
def start_adventure(player_name):
    """
    Displays the intro text for the beginning of the story and prompts player
    to confirm they are ready to start the choose-your-own adventure.
    """
...
def get_story_choice():
    """
    Function to get story choice input from player.
...
def display_spider_story():
    """
    Displays narrative text from story_text file to set the scene for spider
    section. Displays options for player to choose between to direct the
...
def display_spider_story_choices():
    ...
    option and for the choice to be validated (validate_story_choice).

    If valid, the choice is returned to this function for the program to
    display the next block of story text depending on the chosen outcome along
    with some ASCII art.
    """
...
def display_nested_spider_choices():
    ...
    If valid, the choice is returned to this function for the program to
    display the next block of story text depending on the chosen outcome with
    some ASCII art.
    """
    ss.story_separator()
...
def display_centipede_story():
    """
    Displays narrative text from story_text file to set the scene for centipede
    section. Displays options for player to choose between to direct the
    narrative and runs the display_centipede_story_choices function for the
...
def display_rake_story():
    """
    Displays narrative text from story_text file to set the scene for rake
    section. Displays options for player to choose between to direct the
    narrative and runs the display_spider_story_choices function for the player
    to make their choice.
    """
...
def get_restart_choice():
    """
    Function to get choice input from player if they want to restart or finish.
    Run a while loop to collect a valid entry of y or n from the player
...
def validate_restart_choice(value):
    """
    Validates if the player enters a restart_choice value that is 'y' or 'n'.
    Inside the try, raises a ValueError if the player enters a value that is
    not y or n.
    """
    try:
